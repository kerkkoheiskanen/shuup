...
def convert_string(obj):
    ...
    if isinstance(obj, models.Model):
        return convert_string({
            "model": "{}.{}".format(obj._meta.app_label, obj._meta.model.__name__),
            "id": obj.pk
        })
    ...
    try:
        script.add_log_entry(
            ...
            extra=dict(
                ...
                values=convert_string(event.variable_values)
            )
        )
...
notification_sent.connect(
    log_notification_sent, sender=Script, dispatch_uid="log_notification_sent"
)